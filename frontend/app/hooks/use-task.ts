import type { CreateTaskFormData } from "@/components/tasks/create-task-dialog";
...
export const useCreateTaskMutation = ()=>{
    ...
    return useMutation({
        mutationFn: (data: { projectId : string, taskData: CreateTaskFormData})=> postData(`/tasks/${data.projectId}/create-task`,data.taskData),
        onSuccess: (data:any)=>{
...
export const useCreateDescriptionMutation = ()=>{
    const queryClient = useQueryClient();
...
export const useDeleteTaskMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data: any) => {
      // invalidate lists that could be affected
      queryClient.invalidateQueries({ queryKey: ["task", data?.taskId || data?._id] });
