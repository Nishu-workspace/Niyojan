...
const updateTaskTitle = async (req, res) => {
  try {
    ...
    if (!isMember) {
      return res.status(403).json({
        message: "You are not the member of the project",
      });
    }
    const oldTitile = task.title;
    task.title = title;
    await task.save();
    await recordActivity(req.user._id, "updated_task", "Task", taskId, {
      description: `Updated Task title from ${oldTitile} to ${title}`,
    });
...
const updateTaskDescription = async (req, res) => {
  try {
    ...
    const oldDescription = task.description.substring(
      0,
      50 + (task.description.length > 50 ? "..." : "")
    );

    const newDescription = description.substring(
      0,
      50 + (description.length > 50 ? "..." : "")
    );
